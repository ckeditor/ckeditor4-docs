...
The release version of CKEditor is a CKEditor package that was processed by CKBuilder in order to reduce the number of files and minify the resulting code. The release version is offered for download on [CKEditor download page](http://ckeditor.com/download) and by the [CKEditor online builder](http://ckeditor.com/builder) by default.

### Source Version

Without the build process, the CKEditor "full" package would require over 250 files (HTTP requests) to run due to having to load separate plugin files, language files and icons. The "source" version of CKEditor is a version that you can download from the Git repository; it consists of hundreds of files. This is the version on which patches can be applied and as mentioned earlier, it is very unlikely that you are using it.

### How to Check Which Distribution I Use?

Check the size of the ckeditor.js file located in the ckeditor folder that is installed on your website. If the size of that file is larger than 50KB, then you are using the release version.
...
The ckeditor-presets repository is used by the CKEditor team to build the basic/standard/full distributions. It uses ckeditor-dev as a dependency and scripts included there automate the build process further:
...
If you are reading this complex document, most probably you already know which feature you want to port. Whenever you find a change that you need to port, it is recommended to find in which ticket on the development site the change has been tracked.
...
By looking into the [changelog](http://ckeditor.com/whatsnew) you find a link to [ticket 12311](http://dev.ckeditor.com/ticket/12311). The ticket not only explains what the problem was and how to reproduce it, but at the end it also contains a link to a changeset where the change was introduced: [http://github.com/ckeditor/ckeditor-dev/commit/b373ace](http://github.com/ckeditor/ckeditor-dev/commit/b373ace)
...
If you for any reason cannot use Git, you have to apply the changes manually. It is highly unrecommended though. Be careful when changing the files manually.

1. View the changes on GitHub using the link found in a ticket: [http://github.com/ckeditor/ckeditor-dev/commit/b373ace](http://github.com/ckeditor/ckeditor-dev/commit/b373ace)
...
Note that the **line numbers** displayed on GitHub and in your editor **might be different** due to different versions on which you work and to which the change has been applied. In worst case it may turn out that the lines of code that have been changed do not even exist. In such case upgrading and testing will require less effort than continuing with patching.

#### With Git


As mentioned earlier, this is not a Git manual, so we will continue the instructions for the previous example.
...
Fortunately as you are sure that this file is not required by CKEditor, you can apply the patch ignoring that single file:

```
git apply -v 12311.patch --exclude=CHANGES.md
```

Alternatively, when the patch does not apply cleanly, you may fall back on 3-way merge with the --3way flag. Git will then leave the conflict markers in the files in the working tree for the user to resolve:
...
In order to be able to create the same build that you used so far, you should take `build-config.js` from the root folder of your CKEditor distribution that you are still using on production and overwrite `dev/builder/build-config.js` with correct `build-config.js`.

### Add Missing Plugins to the plugins Folder

It is possible that ckeditor-dev/plugins does not have all the plugins that your build had. Before building, you need to verify `build-config.js` and check whether each plugin listed there exists in the plugins folder.
...
If the build that you used had spell checker plugins (scayt/wsc), then you need to copy them as well. Spell checker plugins must be downloaded with the proper revision. The revision of scayt and wsc plugin that was used with release versions of CKEditor can be checked in the ckeditor-presets repository

1. Open https://github.com/ckeditor/ckeditor-presets

2. Press the "branch" select list and then select the Tags tab
...
6. Click both links to exact revisions of scayt and wsc plugins. You will be redirected to their project pages, where the "Download ZIP" option on the right side will return the correct version of each plugin.

    {@img patching6.png}

7. Unpack the downloaded plugins, renaming their folders to scayt and wsc, respectively.

#### 3rd Party Plugins from the Addons Repository


If you used 3rd party plugins from the addons repository, make sure you download them from the addons repository http://ckeditor.com/addons/plugins/all, taking the appropriate versions.
...
5. Install the dependencies that are required in order to test CKEditor

    ```
    npm install
    ```
    Do not use sudo or the global flag here.
    <br>
...
For more information about the testing environment, check the [Testing Environment](#!/guide/dev_tests) article.
