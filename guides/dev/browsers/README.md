...
The CKEditor core team uses an approach of [progressive enhancement](http://en.wikipedia.org/wiki/Progressive_enhancement) to provide the best possible experience on modern, fully capable browsers as well as on older browsers or browsers which do not support all required features. Therefore, the general level of support differs depending on an environment used and some features may not be available in certain cases.

## Officially Supported Browsers

The list of officially supported browsers contains those which the CKEditor core team actively uses during the development and testing. CKEditor may also work in browsers which are not included in the following list, because due to number of various environments the team is not able to check all of them.
...
These browsers are generally fully compatible with just a few features which are not available in them. Namely:

* There is no support for uploading pasted and dropped files (required APIs are not available).
* In Internet Explorer 8.0 the Mathematical Formulas, Code Snippet, Embed and Semantic Embed widgets are not supported.
* In Internet Explorer 8.0 there is no support for nested widgets.
...
As of June 2015, due to current stability issues of Microsoft Edge, the core team was only able to check that CKEditor 4.5 works in this browser, but was not able to address some of the encountered issues. The compatibility will be actively checked and will be improved together with next Microsoft Edge releases.
...
<p class="tip">Note: Prior to version 4.5 CKEditor is disabled in some versions of Chrome for Android and may be disabled in Internet Explorer Mobile for Windows Phone. You can <a href="#!/guide/dev_unsupported_environments">change CKEditor settings to accept any mobile environment</a> (at your own risk).</p>
...
**Note:** Prior to version 4.5 CKEditor was disabled in all environments which were not recognized as compatible. Since CKEditor 4.5 the whitelist was changed to a blacklist, so currently CKEditor is only disabled in environments which it is known to be incompatible with (for example Internet Explorer 7 and below and Firefox 3.6 and below). Read more in {@link CKEDITOR.env.#isCompatible}.
